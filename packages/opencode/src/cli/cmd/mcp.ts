...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in quickcode.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .quickcode/ subdirectory too)
  const candidates = [path.join(baseDir, "quickcode.json"), path.join(baseDir, "quickcode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".quickcode", "quickcode.json"), path.join(baseDir, ".quickcode", "quickcode.jsonc"))
  }
  ...
  // Default to quickcode.json if none exist
  return candidates[0]
}
