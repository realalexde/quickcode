...
function globalConfigFile() {
  const candidates = ["quickcode.jsonc", "quickcode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.QUICKCODE_CONFIG && !Flag.QUICKCODE_CONFIG_DIR && !Flag.QUICKCODE_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "quickcode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "quickcode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "QUICKCODE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.QUICKCODE_CONFIG) {
          yield* merge(Flag.QUICKCODE_CONFIG, yield* loadFile(Flag.QUICKCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.QUICKCODE_CONFIG })
        }

        if (!Flag.QUICKCODE_DISABLE_PROJECT_CONFIG) {
        ...
        if (Flag.QUICKCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from QUICKCODE_CONFIG_DIR", { path: Flag.QUICKCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".quickcode") || dir === Flag.QUICKCODE_CONFIG_DIR) {
            for (const file of ["quickcode.json", "quickcode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.quickcode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.QUICKCODE_CONFIG_CONTENT) {
          const source = "QUICKCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.QUICKCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from QUICKCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["QUICKCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("QUICKCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.QUICKCODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.QUICKCODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("QUICKCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.QUICKCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.QUICKCODE_DISABLE_PRUNE) {
