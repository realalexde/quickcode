...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from quickcode.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...
    ...(await Filesystem.findUp(["quickcode.json", "quickcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "opencode"))
  }
  if (Flag.QUICKCODE_CONFIG) files.push(Flag.QUICKCODE_CONFIG)
