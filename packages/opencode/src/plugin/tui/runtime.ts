...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".quickcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".quickcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".quickcode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.QUICKCODE_PURE ? [] : pluginOrigins
    if (Flag.QUICKCODE_PURE && pluginOrigins.length) {
    }
