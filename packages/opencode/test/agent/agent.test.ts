...
it.instance("plan agent denies edits except .quickcode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".quickcode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".quickcode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.QUICKCODE_TEST_HOME
      process.env.QUICKCODE_TEST_HOME = test.directory
