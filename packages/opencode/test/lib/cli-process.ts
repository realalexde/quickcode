...
//   - QUICKCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - QUICKCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - QUICKCODE_DISABLE_PROJECT_CONFIG : skip walking up for quickcode.json
//   - QUICKCODE_PURE                : skip external plugin discovery + install
//   - QUICKCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    QUICKCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    QUICKCODE_CONFIG_CONTENT: configJson,
    QUICKCODE_DISABLE_PROJECT_CONFIG: "1",
    QUICKCODE_PURE: "1",
    QUICKCODE_DISABLE_AUTOUPDATE: "1",
    QUICKCODE_DISABLE_AUTOCOMPACT: "1",
    QUICKCODE_DISABLE_MODELS_FETCH: "1",
    QUICKCODE_AUTH_CONTENT: "{}",
  }
}
  ...
  return Effect.gen(function* () {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          QUICKCODE_CONFIG_CONTENT: JSON.stringify({
