...
const originalEnv = {
  QUICKCODE_AUTH_CONTENT: process.env.QUICKCODE_AUTH_CONTENT,
  QUICKCODE_EXPERIMENTAL_WORKSPACES: process.env.QUICKCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.QUICKCODE_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.QUICKCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.QUICKCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.QUICKCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.QUICKCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
