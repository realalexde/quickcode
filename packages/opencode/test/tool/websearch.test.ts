...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.QUICKCODE_WEBSEARCH_PROVIDER

    try {
      process.env.QUICKCODE_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.QUICKCODE_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.QUICKCODE_WEBSEARCH_PROVIDER
      else process.env.QUICKCODE_WEBSEARCH_PROVIDER = original
    }
  })
