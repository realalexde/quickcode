...
const original = {
  QUICKCODE_SERVER_PASSWORD: Flag.QUICKCODE_SERVER_PASSWORD,
  QUICKCODE_SERVER_USERNAME: Flag.QUICKCODE_SERVER_USERNAME,
  envPassword: process.env.QUICKCODE_SERVER_PASSWORD,
  envUsername: process.env.QUICKCODE_SERVER_USERNAME,
}
const auth = { username: "opencode", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.QUICKCODE_SERVER_PASSWORD = original.QUICKCODE_SERVER_PASSWORD
  Flag.QUICKCODE_SERVER_USERNAME = original.QUICKCODE_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.QUICKCODE_SERVER_PASSWORD
  else process.env.QUICKCODE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.QUICKCODE_SERVER_USERNAME
  else process.env.QUICKCODE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.QUICKCODE_SERVER_PASSWORD = auth.password
  Flag.QUICKCODE_SERVER_USERNAME = auth.username
  process.env.QUICKCODE_SERVER_PASSWORD = auth.password
  process.env.QUICKCODE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.QUICKCODE_SERVER_PASSWORD = undefined
  Flag.QUICKCODE_SERVER_USERNAME = auth.username
  delete process.env.QUICKCODE_SERVER_PASSWORD
  process.env.QUICKCODE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    await using tmp = await tmpdir({
      init: async (directory) => {
        ...
        await Bun.write(
          path.join(directory, "quickcode.json"),
          JSON.stringify({ formatter: false, lsp: false, plugin: [pathToFileURL(plugin).href] }),
        )
        return { initialized, completed }
      },
    })
    const previous = process.env.QUICKCODE_DISABLE_DEFAULT_PLUGINS
    process.env.QUICKCODE_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.QUICKCODE_DISABLE_DEFAULT_PLUGINS
      else process.env.QUICKCODE_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
