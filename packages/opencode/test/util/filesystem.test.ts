...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "quickcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "quickcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "quickcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "quickcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["quickcode.json", "quickcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "quickcode.json"),
        path.join(project, "quickcode.jsonc"),
      ])
    })
  })
