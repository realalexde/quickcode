...
const APP_IDS: Record<string, string> = {
  dev: "ai.quickcode.desktop.dev",
  beta: "ai.quickcode.desktop.beta",
  prod: "ai.quickcode.desktop",
}
const TEST_ONBOARDING = process.env.QUICKCODE_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = process.env.QUICKCODE_SIDECAR_V2 === "1" ? "v2" : "v1"
...
const main = Effect.gen(function* () {
  ...
  process.env.QUICKCODE_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.quickcode.desktop.dev"
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.QUICKCODE_PORT
      if (fromEnv) {
