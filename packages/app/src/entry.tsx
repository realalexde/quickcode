...
const getCurrentUrl = () => {
  ...
  if (import.meta.env.DEV)
    return `http://${import.meta.env.VITE_QUICKCODE_SERVER_HOST ?? "localhost"}:${import.meta.env.VITE_QUICKCODE_SERVER_PORT ?? "4096"}`
  ...
  Sentry.init({
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" && !(import.meta.env.QUICKCODE_CHANNEL === "prod" && i.name === "GlobalHandlers"),
      )
    },
  })
}
