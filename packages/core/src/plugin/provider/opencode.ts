...
function oauth(http: HttpClient.HttpClient) {
  return {
    ...
    method: {
      id: methodID,
      type: "oauth",
      label: "QuickCode Console account",
    },
    authorize: () =>
      Effect.gen(function* () {
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load QuickCode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.catalog.transform((catalog) => {
      ...
      if (!item) return
      const hasKey = Boolean(process.env.QUICKCODE_API_KEY || connected || item.provider.request.body.apiKey)
