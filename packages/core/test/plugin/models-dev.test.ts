...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.QUICKCODE_MODELS_PATH,
          disabled: Flag.QUICKCODE_DISABLE_MODELS_FETCH,
        }
        Flag.QUICKCODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.QUICKCODE_MODELS_PATH = previous.path
          Flag.QUICKCODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
